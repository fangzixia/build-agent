...
9. **"哪些需要一起改"类问题**：计划中必须包含读取足够代码/文件的步骤，目标是输出具体的联动关系（改 A 需同步改 B、C，原因是共用了什么），而不是分类汇总
...
7. **回答"哪些需要一起改/调整/同步"类问题时**，必须给出具体的联动关系，而不是分类列表：
   - 错误示范：列出报表分组表格，末尾说"如需深入分析请告知"
   - 正确示范：明确指出"改动 X 时，需要同步检查 A、B、C，原因是它们共用了 [具体逻辑/字段/组件]"
   - 必须读取足够的代码/文件来支撑结论，不能停留在分类层面
   - 结论必须可操作：用户拿到答案后，不需要再追问就能知道下一步改哪些文件
