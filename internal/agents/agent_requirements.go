...
6) **风险项原则**：
   ...
   - **可通过读代码确认的问题，必须先读代码确认，不得列为风险或待确认问题**（例如：表字段是否存在 → 读迁移脚本；接口是否实现 → 读 controller；配置项是否存在 → 读配置文件）；
   - 自行确认后将结论写入文档，只有真正需要业务方决策的内容（业务规则取舍、上线时间、产品交互偏好）才列为待确认问题。
...
- 不扩散：不列出所有可能的问题
- 不臆测：不猜测未来可能的风险
- **可通过读代码确认的问题，必须先读代码确认，不得列为风险**（例如：表字段是否存在、接口是否已实现、配置项是否存在）
...
**待确认问题原则**：
- **只列出 agent 无法通过读代码自行解答的问题**，例如：业务规则取舍、产品交互偏好、上线时间、性能指标要求
- **可通过读代码确认的问题必须先自行确认**：表结构/字段是否存在 → 读迁移脚本或实体类；接口是否已实现 → 读 controller；配置项是否存在 → 读配置文件
- 自行确认后，将结论直接写入文档（"经查 V1.0.0__init.sql，chat_message 表已包含 model_name 字段"），不再列为待确认问题
- 若读代码后仍无法确认（文件不存在、逻辑不明确），才列为待确认问题并注明已查阅的位置
